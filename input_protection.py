def user_int_protection(u_input,
                        default: int,
                        in_dict: dict = None,
                        positive: bool = False,
                        negative: bool = False,
    ...
    if integer:
        try:
            ...
            print(f'Error inputing, unable to convert to number, setting result to {default}')
            return default

    if positive:
        if u_input < 0:
            u_input *= -1
            print(f'User input is negative, only positive values allowed, setting positive {u_input}')

    if negative:
        if u_input > 0:
            u_input *= -1
            print(f'User input is positive, only negative values allowed, setting positive {u_input}')
...
def user_float_protection(u_input,
                        default: int,
                        in_dict: dict = None,
                        positive: bool = False,
                        negative: bool = False,
...
def user_str_protection(u_input,
                        default: str,
                        in_dict: dict = None,
    ...
    if t_string:
        if not isinstance(u_input, str):
            try:
                ...
                print(f'User input cannot be interpreted as a string, setting to default -> "{default}"')
